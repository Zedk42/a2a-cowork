...
Layout assumption: this skill directory lives inside the project repo, with
the worker source at `../a2a-worker` relative to it.

## Tooling

`api.py` in this directory is the CLI for everything. It needs `pyyaml`, which
lives in the worker's venv — so invoke it as:

- macOS / Linux: `../a2a-worker/.venv/bin/python api.py ...`
- Windows: `..\\a2a-worker\\.venv\\Scripts\\python api.py ...`

It reads `worker.yaml` for the server URL, domain token and your agent id.
...
Start the worker **in the background** (running it in the foreground would
block your turn):

- macOS / Linux: `cd ../a2a-worker && nohup ./start.sh > ~/a2a-worker.log 2>&1 &`
- Windows: `start /b cmd /c "cd ..\\a2a-worker && start.cmd > %USERPROFILE%\\a2a-worker.log 2>&1"`
...
Config changes (policy, driver, notify id) require a restart: stop the
running pid first (`kill <pid>` / `taskkill /PID <pid> /F`) — the pid is in
`~/.a2a-worker/<agent-id>.lock` — otherwise the old process silently wins.
Re-registering is safe: it overwrites and immediately revives the agent.
Updating the worker code follows the same order — stop the old process, replace the
files, start again (it re-registers itself). An un-killed old process keeps running the
old code.
